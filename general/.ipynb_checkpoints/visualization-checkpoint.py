...
def erode_image(image, e_kernel = np.ones((3,3),np.uint8), iterations = 1):
...
# def mask_image(original, mask, color = [255,0,0], alpha = 0.4):
#     original = array_3channels(original)
#     w,h,c = original.shape
#     x_coord, y_coord = np.where(mask > 0)
#     mask_layer = np.zeros_like(original)
#     for x,y in zip(x_coord, y_coord):
#         mask_layer[x, y] = color
#     blend = (original*(1.0-alpha) + mask_layer*alpha).astype(np.uint8)
#     return blend
...
# def visualize_anomalies(original, reconstruction):
...
#     diff = get_difference(original, reconstruction) # diff should always be grayscale w,h [0,255]
