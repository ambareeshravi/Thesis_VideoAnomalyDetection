...
class CLSTM_AE(nn.Module):
    def __init__(
        ...
        self.__name__ = "CLSTM_AE_%d_%dx%d_L%d_RL%d_DisDeConv%s"%(image_size, most_common(filter_sizes), most_common(filter_sizes), len(filter_count), n_rnn_layers, "Y" if disableDeConvRNN else "N")
        self.channels = channels
        self.image_size = image_size
        self.filter_count = filter_count
        self.filter_sizes = filter_sizes
