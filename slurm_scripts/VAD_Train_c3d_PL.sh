...
#SBATCH --output=../../../projects/def-karray/a24ravi/slurm_outputs/PL_C3D_%u-%x-%j.txt

#SBATCH --mail-user=ambareesh.ravi@uwaterloo.ca
#SBATCH --mail-type=ALL

version=PL_C3D_$(date +%d_%m_%Y_%H_%M)
...
cp run_config_c3d_PL.py $version_path
python run_config_c3d_PL.py --model_path $version_path --data_path $SLURM_TMPDIR/ --nodes 1
echo "[STATUS] Script completed at `date`" 
 
cp ${project_dir}slurm_outputs/PL_C3D_%u-%x-%j.txt $version_path
