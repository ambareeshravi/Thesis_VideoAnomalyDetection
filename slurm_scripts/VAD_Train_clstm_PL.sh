...
#SBATCH --time=0-28:00      # time (DD-HH:MM)
...
python run_config_clstm_PL.py --model_path $SLURM_TMPDIR/Models/ --data_path $SLURM_TMPDIR/ --nodes 1
echo "[STATUS] Script completed at `date`" 

for d in $SLURM_TMPDIR/Models/*/; do cp run_config_clstm_PL.py "$d"; done

tar cf ~/projects/def-karray/a24ravi/trained_models/CLSTM_PL_`date +%d_%m_%Y_%H_%M.tar` $SLURM_TMPDIR/Models/*
