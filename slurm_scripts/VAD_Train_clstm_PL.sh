...
#SBATCH --nodes=1
#SBATCH --gres=gpu:p100:2   # Request GPU "generic resources" [--gres=gpu:2] t4:2
#SBATCH --cpus-per-task=32  # Cores proportional to GPUs: 6 on Cedar, 16 on Graham. [--ntasks-per-node=32] # 44
#SBATCH --mem=120G        # Memory proportional to GPUs: 31500 Cedar, 63500 Graham. [--mem=127G ]

#SBATCH --time=0-36:00      # time (DD-HH:MM)
#SBATCH --output=../../../projects/def-karray/a24ravi/slurm_outputs/PL_CLSTM_%u-%x-%j.txt

#SBATCH --mail-user=ambareesh.ravi@uwaterloo.ca
#SBATCH --mail-type=ALL

version=PL_CLSTM_$(date +%d_%m_%Y_%H_%M)
...
cp run_config_clstm_PL.py $version_path
python run_config_clstm_PL.py --model_path $version_path --data_path $SLURM_TMPDIR/ --nodes 1
echo "[STATUS] Script completed at `date`" 
 
cp ${project_dir}slurm_outputs/PL_CLSTM_%u-%x-%j.txt $version_path
